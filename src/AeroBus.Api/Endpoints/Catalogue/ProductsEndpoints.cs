...
namespace AeroBus.Api.Endpoints.Catalogue
{
    ...
    {
        public static RouteGroupBuilder CatalogueProductsMapping(this RouteGroupBuilder group)
        {
            group.MapGet("/{id:guid}", async (
                Guid id,
                [FromServices] ProductsService svc,
                ...
                ClaimsPrincipal user) =>
            {
                try
                {
                    product = product with { CompanyId = user.ResolveCompanyId(product.CompanyId) };
                    var saved = await svc.SaveAsync(product);
