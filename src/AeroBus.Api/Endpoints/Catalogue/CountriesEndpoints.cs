...
namespace AeroBus.Api.Endpoints.Catalogue
{
    ...
    {
        public static RouteGroupBuilder CatalogueCountriesMapping(this RouteGroupBuilder group)
        {
            group.MapGet("/{id:guid}", async (
                Guid id,
                [FromServices] CountriesService svc,
            ...
            // Main list endpoint – paged + search + optional continent/status
            group.MapGet("/", async (
                [FromServices] CountriesService svc,
                ClaimsPrincipal user,
                [FromQuery] Guid? continentId,
                ...
                [FromQuery] int? pageSize) =>
            {
                ...
                var items = await svc.ListByCompanyAsync(companyId, continentId, status, search, page, size);
            ...
            // Non-paged "by-continent" (not company scoped)
            group.MapGet("/by-continent/{continentId:guid}", async (
                Guid continentId,
                [FromServices] CountriesService svc,
                ClaimsPrincipal user) =>
            {
                var items = await svc.GetByContinentAsync(continentId);
                return Results.Ok(items);
            });
